...
class FtpLimiter extends Transform {
    ...
    _transform (chunk, encoding, callback) {
        ...
        if (!this.limiting) {
            ...
            this.emit('progress', this.transferred, size, this);
        ...
        if (this.limiting) {
            ...
            if (size > this.realRate) {
                ...
                if (!slice.length) {
                    ...
                    this.emit('progress', this.transferred, slice.length, this);
                ...
                this.push(chunk);
                this.emit('progress', this.transferred, chunk.length, this, true);
                callback();
            }
        } else {
            this.transferred += chunk.length;
            this.emit('progress', this.transferred, chunk.length, this);
            // this.log('LIMITER, limiting off, size: ' + chunk.length + ', total: ' + this.transferred);
            callback(null, chunk);
        }
    }
