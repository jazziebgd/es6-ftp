...
class FtpLimiter extends Transform {
     ...
     */
    onEnd() {
        this.ftpRequest.log('LIMITER:onEnd');
        this.complete();
    }
     ...
     */
    onClose() {
        this.ftpRequest.log('LIMITER:onClose');
    }
     ...
     */
    onDrain() {
        this.ftpRequest.log('LIMITER:onDrain');
        // this.resume();
    }
     ...
     */
    onProgress() {
        // this.ftpRequest.log('LIMITER:onProgress');
        if (this.ftpRequest) {
            this.ftpRequest.transferred = this.transferred;
        }
    }
     ...
     */
    onPipe() {
        this.ftpRequest.log('LIMITER:onPipe');
    }
     ...
     */
    onFinish() {
        this.ftpRequest.log('LIMITER:onFinish');
        this.complete();
    }
    ...
    complete () {
        ...
        if (!this.completed) {
            this.ftpRequest.log('LIMITER:complete');
            this.completed = true;
     ...
     */
    _flush (done) {
        this.ftpRequest.log('LIMITER:flush');
        done();
    }

    /**
     * Process method implementation that limits transfer speed
     *
     * @param  {Buffer}   chunk    Data chunk buffer
     * @param  {String}   encoding Data chunk encoding
     * @param  {Function} callback Callback method
    ...
    process (chunk, encoding, callback) {
        ...
        if (!this.limiting) {
            ...
            this.ftpRequest.log('LIMITER.process, no limit, size: ' + chunk.length + ', total: ' + this.transferred);
            ...
            if (size > this.realRate) {
                ...
                if (!slice.length) {
                    this.ftpRequest.log('LIMITER.process, limiting end?, total: ' + this.transferred);
                    callback(null, slice);
                } else {
                    this.ftpRequest.log('LIMITER.process, limiting, size: ' + slice.length + ', total: ' + this.transferred);
                    this.transferred += slice.length;
                    this.emit('progress', this.transferred);
                    let rest = chunk.slice(this.realRate);
                    this.push(slice);
                    this.timeout = setTimeout( () => {
                        this.ftpRequest.log('LIMITER.process, calling itself, total: ' + this.transferred);
                        this.process(rest, encoding, callback);
                    }, this.duration);
                }
            } else {
                this.ftpRequest.log('LIMITER.process, limiting but chunk is smaller than rate, total: ' + this.transferred);
                this.transferred += chunk.length;
                this.emit('progress', this.transferred);
                this.push(chunk);
                callback();
