...
class FtpClient extends FtpBase {
    ...
    async setSecurity () {
        ...
        if (this.connection.secure) {
            try {
                let ftpRequest = await this.send('AUTH TLS');
    ...
    async getFeatures() {
        ...
        let feat = await this.sendImmediate('FEAT');
        if (feat && feat.text) {
    ...
    async login() {
        ...
        if (this.connection && this.connection.user && this.connection.password) {
            ...
            try {
                userResponse = await this.send('USER ' + username);
            } catch (ex) {
            ...
            if (userResponse && userResponse.responseCode && userResponse.responseCode == 331) {
                ...
                let passResponse;
                try {
                    passResponse = await this.send('PASS ' + password);
                } catch (ex) {
    ...
    async startPassive(resetCount = true) {
        ...
        try {
            ...
            if (passiveSocket) {
                this.isPassive = true;
                this.logDebug('Passive mode established, ip:' + passiveInfo.ip + ', port: ' + passiveInfo.port);
            } else {
    ...
    createFtpRequest(command = '', encoding = null, active = false, pending = true, finished = false, responseCode = null, response = null, text = null, error = false, errorMessage = '') {
        ...
        let ftpRequest = new frClass(command, encoding, active, pending, finished, responseCode, response, text, error, errorMessage);
    ...
    /**
     * Executes command immediately and returns its ftpRequest instance
     *
     * @async
     * @throws {Error} If request fails
     *
    ...
    async executeCommand(command, encoding = null, ignoreBusy = false) {
        ...
        if (!this.busy || ignoreBusy) {
            ...
            let _sendImmediate = (resolve, reject) => {
                let socket = this.getSocket();
                let onSendResponse = (result) => {
                    ftpRequest.responseCode = result.responseCode;
                    ftpRequest.response = result.text;
                    ftpRequest.text = result.text;
                    this._currentBuffer = '';
            ...
            return new Promise(_sendImmediate);
        } else {
            ...
            return new Promise( async (resolve, reject) => {
                this.once('free', async () => {
                    console.log('isfree');
                    try {
                        let result = await this.sendImmediate(command, encoding);
                        resolve(result);
                    } catch (ex) {
                        reject(ex);
                    }
    ...
    async send (command, encoding = null, prepend = false) {
        ...
        if (ftpRequest) {
            // ftpRequest.once('unqueue', () => {
            //     console.log('unq', ftpRequest.command);
            // });
            // ftpRequest.once('finish', () => {
            //     console.log('fin', ftpRequest.command);
            // });
            return new Promise( (resolve, reject) => {
            ...
            if (!this.busy) {
                ...
                let onSendResponse = (result) => {
                    ...
                    if (result.responseCode && result.responseCode >= 500) {
                        ftpRequest.setError(result.text);
                        reject(new Error(result.text));
                    }
                    this.removeObjListener(socket, 'error', onSendError);
                    // this.currentLimiter = null;
                    this.setBusy(false);
                ...
                let onSendError = async (error) => {
                    ...
                    // this.currentLimiter = null;
                    this.setBusy(false);
                    await this.send('MODE S');
                    reject(error);
                };

                let passiveSocket = this.getPassiveSocket();
                passiveSocket = this.limitTransferSpeed(passiveSocket, ftpRequest, this.options.limitDownload);
                let source = passiveSocket;
                let canCompress = false;
                if (this.options.compression && this.hasFeature('MODE Z')) {
                    try {
                        let modeZResponse = await this.send('MODE Z');
            ...
            if (ftpRequest) {
                if (!activeFtpRequest) {
                    if (!this.busy) {
                        ...
                        this.logWarning('bussyyyyy ');
                    }
                } else {
                    this.logWarning('There already is an active request ("' + this.sanitizeCommand(activeFtpRequest.command) + '"), not processing queue ("' + this.sanitizeCommand(ftpRequest.command) + '")');
                    return false;
                }
            } else {
            ...
            if (!isUpload) {
                let _processFtpRequest = async (resolve, reject) => {
                    ...
                    if (passiveSocket) {
                        ...
                        ftpRequest.setPassiveSocket(passiveSocket);
                        ftpRequest.setActive();
                        try {
                ...
                let _processUploadFtpRequest = async (resolve, reject) => {
                    ...
                    if (passiveSocket) {
                        ...
                        // let storRequest = await this.send(ftpRequest.command);
                        let storRequest = await this.executeCommand(ftpRequest);
    ...
    async getResponseBool(command, successCodes = [], failureCodes = [], immediate = false) {
        ...
        let ftpRequest;
        if (!immediate) {
            ftpRequest = await this.send(command);
        } else {
            ftpRequest = await this.sendImmediate(this.createFtpRequest(command));
        }
    ...
    async getResponseText(command, successCodes = [], failureCodes = [], immediate = false) {
        ...
        if (!immediate) {
            ...
            ftpRequest = await this.sendImmediate(command);
        }
        if (ftpRequest && ftpRequest.responseCode) {
            if (successCodes && successCodes.length && _.includes(successCodes, ftpRequest.responseCode)) {
    ...
    async abort(force = false) {
        if (!force) {
            ...
            return await this.executeCommand('ABOR', null, true);
        }
    }
