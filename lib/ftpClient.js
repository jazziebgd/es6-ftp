...
class FtpClient extends FtpBase {
    ...
    constructor(options = null) {
        ...
        this.statistics = {
            ...
            activeRequestPercent: null,
            activeRequestTransferred: null,
            activeRequestTotal: null,
        };
        ...
        this.boundMethods = {
            ...
            aborted: this.aborted.bind(this),
    ...
    addEventListeners() {
        ...
        this.on('log', this.boundMethods.onLog);
    }
    ...
    removeEventListeners() {
        ...
        this.removeListener('log', this.boundMethods.onLog);
    }
    ...
    limitTransferSpeed(socket, ftpRequest, limitRate) {
        ...
        this.currentLimiter.setCallbackTimeout(this.options.limiterCallbackTimeout);
        this.currentProgress = 0;
    ...
    handleData (chunk, encoding, callback = null) {
        this.logCall('handleData', _.concat([chunk.toString('binary')], arguments));
    ...
    async onPassiveClose (hadErr) {
        this.logCall('onPassiveClose', arguments);
        if (hadErr) {
     ...
     *
     * @param  {Array}  loggerArgs      Arguments passed to logger
     * @param  {Object} loggerObj       Object that logged the message
     *
     * @return {undefined}
     */
    onLog (loggerArgs, loggerObj) {
        // this.logCall('onLog', loggerArgs, loggerObj);
    ...
    /**
     * Resets ftp statistics data - emits 'statistics:reset' event
     *
     * @return {undefined}
     */
    resetStatistics () {
        ...
        this.statistics.hasActiveRequest = false;
        this.statistics.activeRequestPercent = null;
        this.statistics.activeRequestTransferred = null;
        this.statistics.activeRequestTotal = null;
    ...
    /**
     * Callback for abort - emits 'aborted' event
     *
     * @return {undefined}
     */
    aborted () {
        this.logCall('aborted', arguments);
        this.clearQueue();
        this.aborting = false;
        this.emit('aborted');
    }
    ...
    /**
     * Triggered on 'progress' event on ftpLimiter, logs progress info
     *
     * @param  {Number} transferred Number of bytes transferred
     * @return {undefined}
     */
    ...
    setCurrentSpeed (ftpRequest, speed) {
        if (this.options.calculateStatistics && _.isFinite(speed)) {
    ...
    /**
     * Calculates average speed based on speed sum and measure counts
     *
     * @param  {Number} now Timestamp for history entry
     *
     * @return {undefined}
     */
    calculateAverageSpeed (now) {
        let speedSum = this.intermediateStatistics.speedSum;
        let measureCount = this.intermediateStatistics.measureCount;
        if (this.options.calculateStatistics) {
            if (_.isFinite(this.statistics.currentSpeed)) {
                speedSum += this.statistics.currentSpeed;
                measureCount++;
            }
            let averageSpeed = speedSum / measureCount;
            if ((averageSpeed || averageSpeed === 0) && _.isFinite(averageSpeed)){
                this.statistics.averageSpeed = averageSpeed;
                if (this.options.keepStatisticsHistory) {
                    let addEntry = true;
                    if (this.statistics.averageSpeedHistory.length) {
                        let lastEntry = this.statistics.averageSpeedHistory[this.statistics.averageSpeedHistory.length - 1];
                        if (lastEntry[1] == this.statistics.averageSpeed) {
                            addEntry = false;
                        }
                    }
                    if (addEntry) {
                ...
                this.intermediateStatistics.speedSum = speedSum;
                this.intermediateStatistics.measureCount = measureCount;
            }
        }
    }

    /**
     * Handler for 'queue:complete' event on ftpClient, triggered when current active request finishes
     *
     * @param  {ftpRequest} ftpRequest Instance of ftpRequest that has completed
     * @return {undefined}
     */
    ...
    /**
     * Handler for 'queue:finish' event on ftpClient, triggered when there is no more requests in queue
     *
     * @return {undefined}
     */
    onQueueFinished () {
    ...
    /**
     * Handler for 'queue:add' event on ftpClient, triggered when ftpRequest is added to queue
     *
     * @param  {ftpRequest} ftpRequest Instance of ftpRequest that has been added to queue
     * @return {undefined}
     */
    ...
    /**
     * Handler for 'queue:pause' event on ftpClient, triggered when queue is paused
     *
     * @return {undefined}
     */
    onQueuePause () {
    ...
    /**
     * Handler for 'queue:resume' event on ftpClient, triggered when queue is resumed
     *
     * @return {undefined}
     */
    onQueueResume () {
    ...
    /**
     * Handler for 'queue:remove' event on ftpClient, triggered when ftpRequest is removed from queue
     *
     * @param  {ftpRequest} ftpRequest Instance of ftpRequest that has been removed from queue
     * @return {undefined}
     */
    ...
    async processFtpRequest(ftpRequest) {
        ...
        if (!usePassive) {
            ...
            if (!isUpload) {
                let _processFtpRequest = async (resolve, reject) => {
                    ...
                    if (passiveSocket) {
                        ...
                        try {
                            ...
                            this.activateFtpRequest(ftpRequest);
                            // this.executeCommand(ftpRequest);
                            this.setBusy(true);
                            this.command(ftpRequest.command, ftpRequest.encoding);
                        } catch (ex) {
                            this.removeObjListener(ftpRequest, 'finish', _onRequestFinish);
                ...
                if (this.options.calculateStatistics && ftpRequest.baseCommand == 'RETR' && ftpRequest.params && ftpRequest.params.length) {
                    let size = await this.executeCommand('SIZE ' + ftpRequest.params.join(' '), null, true);
                    if (size && size.text && +size.text) {
                        ftpRequest.inputSize = +size.text;
                    }
                }
    ...
    async abort(force = false) {
        if (!force) {
            await this.getResponseBool('ABOR', [226], [], true);
        } else {
            this.aborting = true;
            this.pauseQueue();
            this.command('ABOR', null);
            this.addOnceObjListener(this, 'queue:remove', this.boundMethods.aborted);
        }
    }
     ...
     */
    async size(ftpPath, immediate = false) {
        let result = false;
        let response = await this.getResponseText('SIZE ' + ftpPath, [213], [], immediate);
        if (response) {
    ...
    activateFtpRequest(ftpRequest, modifyRequest = true) {
        if (modifyRequest) {
            ...
            // this.emit('ftpRequest:active', ftpRequest);
        }
        this.statistics.hasActiveRequest = true;
    ...
    deactivateFtpRequest(ftpRequest, modifyRequest = true) {
        if (modifyRequest) {
            ...
            // this.emit('ftpRequest:inactive', ftpRequest);
        }
        this.statistics.hasActiveRequest = false;
    ...
    async requestFinished(ftpRequest) {
        if (ftpRequest && !ftpRequest.historyDone) {
            ...
            if (this.options.calculateStatistics && this.options.keepStatisticsHistory) {
                if (!ftpRequest.isPassive) {
    ...
    /**
     * Loads item structure for given ftp path
     *
     * @async
     *
     * @param  {String}  ftpPath   Starting path
     * @param  {Boolean} recursive Flag to indicate whether to delve into subdirectories
     * @param  {Boolean} flat      Flag to indicate whether to return children under parents or all items in one-dimensional array
     *
     * @return {FtpFileItem[]}     An array of found file items
     */
    ...
    /**
     * Updates active request statistic data
     *
     * @param  {FtpRequest} activeRequest Active ftp request
     *
     * @return {undefined}
     */
    updateActiveRequestStatistics(activeRequest) {
        this.statistics.activeRequestPercent = activeRequest.percent;
        this.statistics.activeRequestTransferred = activeRequest.transferred;
        this.statistics.activeRequestTotal = activeRequest.inputSize;
    }

    /**
     * Adds history entry for statistics
     *
     * @param {FtpRequest}  ftpRequest     FtpRequest instance for history entry
     * @param {Number}      speed          Speed for history entry
     * @param {Number|Date} time           Date or timestamp for history entry
     * @param {Object}      additionalData Additional data for history entry
     *
     * @return {undefined}
     */
    ...
    /**
     * Gets history entry for given data
     *
     * @param {Number}      speed          Speed for history entry
     * @param {Number|Date} time           Date or timestamp for history entry
     * @param {Object}      additionalData Additional data for history entry
     *
     * @return {Object}                     History entry object
     */
    ...
    /**
     * Adds history entry to upload history
     *
    ...
    /**
     * Adds history entry to download history
     *
     * @param {Object} historyEntry History entry to add
     *
     * @return {undefined}
    ...
    /**
     * Adds zero history entry for statistics (prevents chart from displaying continuous speeds once request finishes)
     *
     * @param {FtpRequest}  ftpRequest     FtpRequest instance for history entry
     * @param {Object}      additionalData Additional data for history entry
     * @param {Number|Date} now            Date or timestamp for history entry
     *
     * @return {undefined}
     */
    ...
    /**
     * Adds history entry to average history
     *
     * @param {Object} historyEntry History entry to add
     *
     * @return {undefined}
    ...
    /**
     * Formats time to normalized values
     *
     * @param  {Number|Date}  date      Date or timestamp to format
     * @param  {Boolean} omitSeconds    Flag to indicate whether to omit seconds from formatted value
     *
