...
class FtpClient extends FtpBase {
    ...
    async setSecurity () {
        ...
        if (this.connection.secure) {
            try {
                let ftpRequest = await this.sendCommandSync('AUTH TLS');
        ...
        if (this.securityState == 'tls') {
            ...
            return new Promise( async (resolve, reject) => {
                let _onSecureConnect = async () => {
                    ...
                    try {
                        let ftpRequest = await this.sendCommandSync('PBSZ 0');
                    ...
                    if (!error) {
                        try {
                            let ftpRequest = await this.sendCommandSync('PROT P');
                            if (!(ftpRequest && ftpRequest.responseCode && ftpRequest.responseCode == 200)) {
                                error = new Error(ftpRequest.response);
                            }
                        } catch (ex) {
    ...
    async getFeatures() {
        ...
        let feat = await this.sendCommandSync('FEAT');
        if (feat && feat.text) {
    ...
    async login() {
        ...
        if (this.connection && this.connection.user && this.connection.password) {
            ...
            try {
                userResponse = await this.sendCommandSync('USER ' + username);
            } catch (ex) {
            ...
            if (userResponse && userResponse.responseCode && userResponse.responseCode == 331) {
                ...
                let passResponse;
                try {
                    passResponse = await this.sendCommandSync('PASS ' + password);
                } catch (ex) {
    ...
    async startPassive() {
        ...
        let ip = '';
        let port = null;
        let response = await this.sendCommandImmediateSync(this.createFtpRequest('PASV'));
     ...
     */
    async sendCommandImmediateSync(command, encoding = null) {
        this.logCall('sendCommandImmediateSync', arguments);
        let ftpRequest;
        if (!this.busy) {
            this.busy = true;
            ...
            let _sendCommandImmediateSync = (resolve, reject) => {
                let socket = this.getSocket();
            ...
            return new Promise(_sendCommandImmediateSync);
        } else {
            return false;
        }
    }
    ...
    async sendCommand(command, encoding) {
        ...
        let ftpRequest = this.queueCommand(command, encoding);
        return ftpRequest;
    }
     ...
     * @return {FtpRequest}                     Ftp request object populated with server response
     */
    async sendCommandSync(command, encoding = null) {
        this.logCall('sendCommandSync', arguments);
        let ftpRequest;
        if (command instanceof FtpRequest) {
            ftpRequest = command;
        } else {
    ...
    async sendFtpRequest(ftpRequest) {
        ...
        let _sendFtpRequest = async (resolve, reject) => {
            ...
            let onSendError = async (error) => {
                this.removeObjListener(this, 'response', onSendResponse);
                ftpRequest.setError(error + '');
                await this.sendCommandSync('MODE S');
                reject(error);
            };
            ...
            if (this.options.compression && this.hasFeature('MODE Z')) {
                try {
                    let modeZResponse = await this.sendCommandSync('MODE Z');
     ...
     */
    queueCommand(command, encoding = null) {
        let ftpRequest;
        if (command instanceof FtpRequest) {
            ftpRequest = command;
        } else {
        ...
        if (this.queue && this.queue.length) {
            ...
            if (!activeFtpRequest) {
                ...
                if (ftpRequest) {
                    ...
                    let removedRequest = _.head(this.queue);
                    _.pullAt(this.queue, 0);
                    this.emit('queue:remove', removedRequest);
                    this.emit('queue:complete', removedRequest);
                    return ftpRequest;
                } else {
    ...
    onQueueComplete (ftpRequest) {
        ...
        this.logQueue('Queued command ' + this.sanitizeCommand(ftpRequest.command) + ' completed');
    }
    ...
    onQueueAdd (ftpRequest) {
        ...
        this.logQueue('Command ' + this.sanitizeCommand(ftpRequest.command) + ' added to queue');
    ...
    onQueueRemove (ftpRequest) {
        ...
        this.logQueue('Command ' + this.sanitizeCommand(ftpRequest.command) + ' removed from queue');
    ...
    async processFtpRequest(ftpRequest) {
        ...
        if (!usePassive) {
            await this.sendCommandImmediateSync(ftpRequest);
            return ftpRequest;
        } else {
            if (!isUpload) {
                let _processFtpRequest = async (resolve, reject) => {
                    // ftpRequest.preventFinish = true;
                    ...
                    if (passiveSocket) {
                        let onPassiveResponse = async (text) => {
                            ...
                            await this.passiveDisconnect();
                            if (ftpRequest.preventFinish) {
                                ftpRequest.emit('finish', null, ftpRequest);
                            }
                            resolve(ftpRequest);
                        };
                        ...
                        let onPassiveError = async (error) => {
                            ...
                            await this.passiveDisconnect();
                            if (ftpRequest.preventFinish) {
                                ftpRequest.emit('finish', error, ftpRequest);
                            }
                            reject(error);
                        };
                        ...
                        try {
                            ...
                            await this.passiveDisconnect();
                            if (ftpRequest.preventFinish) {
                                ftpRequest.emit('finish', ex, ftpRequest);
                            }
                            reject(ex);
                        }
                    } else {
                        ...
                        if (ftpRequest.preventFinish) {
                            ftpRequest.emit('finish', new Error(errorMessage), ftpRequest);
                        }
                ...
                let _processUploadFtpRequest = async (resolve, reject) => {
                    ...
                    if (passiveSocket) {
                        ...
                        // let storRequest = await this.sendCommandSync(ftpRequest.command);
                        let storRequest = await this.sendCommandImmediateSync(ftpRequest);
                        ...
                        if (storRequest && storRequest.responseCode && (storRequest.responseCode === 150 || storRequest.responseCode === 125)) {
                            ...
                            let onfinish = async () => {
                                ...
                                this.removeObjListener(destination, 'finish', onfinish);
                                this.removeObjListener(destination, 'end', onend);
                                // console.log('onfinish', storRequest.command, storRequest.response, storRequest.text);
                                if (canCompress) {
                                    await this.sendCommandSync('MODE S');
                                }
                                await this.passiveDisconnect();
                            ...
                            let onerror = async (err) => {
                                ...
                                await this.passiveDisconnect();
                                if (ftpRequest.preventFinish){
                                    ftpRequest.emit('finish', err, ftpRequest);
                                }
                                reject(err);
                            };
                            ...
                            await this.passiveDisconnect();
                            let err = new Error(storRequest.response);
                            if (ftpRequest.preventFinish){
                                ftpRequest.emit('finish', err, ftpRequest);
                            }
                            reject(err);
                ...
                return new Promise(_processUploadFtpRequest);
            }
        }
    }
    ...
    async getResponseBool(command, successCodes = [], failureCodes = [], immediate = false) {
        ...
        let ftpRequest;
        if (!immediate) {
            ftpRequest = await this.sendCommandSync(command);
        } else {
            ftpRequest = await this.sendCommandImmediateSync(this.createFtpRequest(command));
        }
    ...
    async getResponseText(command, successCodes = [], failureCodes = [], immediate = false) {
        ...
        if (!immediate) {
            ...
            ftpRequest = await this.sendCommandImmediateSync(command);
        }
        if (ftpRequest && ftpRequest.responseCode) {
            if (successCodes && successCodes.length && _.includes(successCodes, ftpRequest.responseCode)) {
    ...
    async list(ftpPath, listAll = false) {
        ...
        command += ftpPath;
        let ftpRequest = this.queueCommand(command);
        return new Promise( (resolve, reject) => {
            let _requestFinish = (err, requestObj) => {
                this.removeObjListener(ftpRequest, 'error', _requestError);
                if (err) {
    ...
    async rename (sourcePath, destinationPath) {
        let result = false;
        let renFromCommand = await this.sendCommandSync('RNFR ' + sourcePath);
        if (renFromCommand && renFromCommand.responseCode && renFromCommand.responseCode == 350) {
            let renToCommand = await this.sendCommandSync('RNTO ' + destinationPath);
    ...
    async lastMod(ftpPath) {
        let result = false;
        let response = await this.getResponseText('MDTM ' + ftpPath, [213]);
        if (response) {
            result = parseInt(response, 10);
        }
        if (isNaN(result)){
    ...
    async pwd () {
        let result = false;
        let ftpRequest = await this.sendCommandSync('PWD');
    ...
    async mkdir (ftpPath, recursive = false) {
        let result = false;
        if (!recursive) {
            let ftpRequest = await this.sendCommandSync('MKD ' + ftpPath);
            if (ftpRequest && ftpRequest.responseCode && ftpRequest.responseCode == 257) {
                result = true;
            } else {
                this.logError(ftpRequest.response);
            ...
            if (this.hasFeature('SITE MKDIR')) {
                let ftpRequest = await this.sendCommandSync('SITE MKDIR ' + ftpPath);
                if (ftpRequest && ftpRequest.responseCode && ftpRequest.responseCode == 200) {
                    result = true;
                } else {
                    this.logError(ftpRequest.response);
    ...
    async rmdir (ftpPath, recursive = false) {
        let result = false;
        if (!recursive) {
            let ftpRequest = await this.sendCommandSync('RMD ' + ftpPath);
            ...
            if (this.hasFeature('SITE RMDIR')) {
                let ftpRequest = await this.sendCommandSync('SITE RMDIR ' + ftpPath);
                if (ftpRequest && ftpRequest.responseCode && ftpRequest.responseCode == 200) {
                    result = true;
                } else {
                    this.logError(ftpRequest.response);
    ...
    async store(command, input, ftpPath) {
        ...
        this.queueCommand(ftpRequest);
        return new Promise( (resolve, reject) => {
            ftpRequest.once('finish', (err, requestObj) => {
                if (err) {
                    reject(err);
                } else if (requestObj.responseCode && (requestObj.responseCode === 150 || requestObj.responseCode === 125)){
                    resolve(true);
                } else {
                    resolve(false);
                }
    ...
    isRequestUpload(ftpRequest) {
        ...
        if (requestPassive && ftpRequest.baseCommand) {
