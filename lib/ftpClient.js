...
class FtpClient extends FtpBase {
    ...
    async get(ftpPath) {
        ...
        ftpRequest.errorCodes.push(550);
        return new Promise( (resolve, reject) => {
            let _requestFinish = (err, requestObj) => {
                this.removeObjListener(ftpRequest, 'error', _requestError);
                if (err) {
    ...
    async fileExists(ftpPath) {
        let result = await this.lastMod(ftpPath, true);
        if (result) {
            result = true;
        }
        return result;
     ...
     */
    async lastMod(ftpPath, returnRaw = false) {
        let result = false;
        let ftpRequest = this.createFtpRequest('MDTM ' + ftpPath);
        ftpRequest.errorCodes.push(550);
        let response = await this.getResponseText(ftpRequest, [213]);
        if (response) {
            result = parseInt(response, 10);
        }
        if (isNaN(result)){
    ...
    async store(command, input, ftpPath) {
        ...
        ftpRequest.errorCodes.push(553);
        ftpRequest.errorCodes.push(451);
