...
class FtpClient extends FtpBase {
    ...
    limitSocketSpeed(socket) {
        ...
        limiter.on('progress', this.boundMethods.onProgress);
    ...
    async startPassive(resetCount = true) {
        ...
        let response = await this.sendImmediate(this.createFtpRequest('PASV'));
     ...
     */
    async sendImmediate(command, encoding = null, ignoreBusy = false) {
        ...
        if (ftpRequest) {
            if (!prepend) {
                ...
                if (!activeFtpRequest) {
                    ...
                    console.log('yes', activeFtpRequest.command);
    ...
    async processFtpRequest(ftpRequest) {
        ...
        if (!usePassive) {
            await this.sendImmediate(ftpRequest);
            return ftpRequest;
        } else {
            if (!isUpload) {
                ...
                let _processUploadFtpRequest = async (resolve, reject) => {
                    ...
                    if (passiveSocket) {
                        ...
                        let storRequest = await this.sendImmediate(ftpRequest, null, true);
