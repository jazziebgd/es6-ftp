...
class FtpClient extends FtpBase {
    ...
    constructor(options = null) {
        ...
        this.boundMethods = {
            ...
            calculateStatisticData: this.calculateStatisticData.bind(this),
        };
    ...
    onProgress (transferred, chunkSize, limiterObject = null, isFinal = false) {
        this.calculateStatisticData(transferred, chunkSize, limiterObject, isFinal);
    ...
    calculateStatisticData(transferred, chunkSize, limiterObject = null, isFinal) {
        // let diff = transferred - this.currentProgress;
        let now = new Date().getTime();
        this.intermediateStatistics.previousTransferred = this.statistics.totalTransferred;
        this.statistics.totalTransferred += chunkSize;
        this.currentProgress = transferred;
        if (this.options.calculateStatistics) {
            let isFirstChunk = !limiterObject.ftpRequest.dataStarted;
            let previousTime = this.intermediateStatistics.previousTime;
            if (previousTime) {
                let duration = now - previousTime;
                if (chunkSize) {
                    limiterObject.ftpRequest.dataStarted = true;
                    if (!isFinal && duration > 80 && !isFirstChunk) {
                        this.calculateCurrentSpeed(now, chunkSize, duration, limiterObject);
                    }
                }
                this.calculateAverageSpeed(now);
            }

            this.intermediateStatistics.previousTime = now;
        }
    }

    calculateCurrentSpeed (now, chunkSize, duration, limiterObject = null) {
        let newCurrentSpeed = (chunkSize / duration) * 1000;
        if ((newCurrentSpeed || newCurrentSpeed === 0) && !isNaN(newCurrentSpeed)){
            this.statistics.currentSpeed = newCurrentSpeed;
            let oldSpeed = 0;
            if (limiterObject && limiterObject.isUpload) {
                oldSpeed = this.statistics.currentUploadSpeed;
                this.statistics.currentUploadSpeed = newCurrentSpeed;
            } else {
                oldSpeed = this.statistics.currentDownloadSpeed;
                this.statistics.currentDownloadSpeed = newCurrentSpeed;
            }



            if (this.options.keepStatisticsHistory) {
                if (limiterObject && limiterObject.ftpRequest && !limiterObject.ftpRequest.historyStarted) {
                    if (!oldSpeed){
                        let additionalZeroData = {
                            command: limiterObject.ftpRequest.id + ' ' + this.sanitizeCommand(limiterObject.ftpRequest.command),
                            isUpload: true
                        };
                        let zeroEntry = this.getHistoryEntry(0, new Date(now - 1), additionalZeroData);
                        if (limiterObject.isUpload) {
                ...
                if (limiterObject) {
                    additionalData.isUpload = limiterObject.isUpload;
                    if (limiterObject.ftpRequest) {
                        additionalData.command = limiterObject.ftpRequest.id + ' ' + this.sanitizeCommand(limiterObject.ftpRequest.command);
                        additionalData.isUpload = limiterObject.ftpRequest.isUpload;
                        additionalData.ftpObject = limiterObject.ftpRequest;
                    }
                }
                let historyEntry = this.getHistoryEntry(newCurrentSpeed, new Date(now), additionalData);
                if (additionalData.isUpload) {
                    this.addCurrentUploadHistoryEntry(historyEntry);
                } else {
                    this.addCurrentDownloadHistoryEntry(historyEntry);
                }
                if (limiterObject && limiterObject.ftpRequest) {
                    limiterObject.ftpRequest.historyStarted = true;
                }
            }
    ...
    async requestFinished(ftpRequest) {
        if (ftpRequest && !ftpRequest.historyDone) {
            ...
            if (this.options.keepStatisticsHistory) {
                if (!ftpRequest.isPassive || !ftpRequest.historyStarted) {
                    ftpRequest.historyStarted = true;
                    // let additionalDataStart = {
                    //     command: ftpRequest.id + ' ' + this.sanitizeCommand(ftpRequest.command),
                    //     isUpload: false
                    // };
                    // let startTime = ftpRequest.startTime;
                    // if (!startTime) {
                    //     startTime = Date.now();
                    // }
                    // let speed = ftpRequest.speed;
                    // if (speed > 0 && speed < 1) {
                    //     speed = 1;
                    // }
                    // let historyEntry = this.getHistoryEntry(speed, new Date(startTime), additionalDataStart);
                    // if (ftpRequest.isUpload) {
                    //     this.addCurrentUploadHistoryEntry(historyEntry);
                    // } else {
                    //     this.addCurrentDownloadHistoryEntry(historyEntry);
                    ...
                    let additionalDataEnd = {
                        command: ftpRequest.id + ' ' + this.sanitizeCommand(ftpRequest.command),
                        isUpload: false
                    };
                    let endTime = ftpRequest.endTime;
                    if (!endTime) {
                        endTime = Date.now() + 1;
                    }
                    let zeroEntry = this.getHistoryEntry(0, new Date(endTime), additionalDataEnd);
                    if (ftpRequest.isUpload) {
                        this.addCurrentUploadHistoryEntry(zeroEntry);
                    } else {
                        this.addCurrentDownloadHistoryEntry(zeroEntry);
                    }
                } else if (ftpRequest.isPassive) {
                    let nowDate = Date.now();
                    let additionalData = {
                        command: ftpRequest.id + ' ' + this.sanitizeCommand(ftpRequest.command),
                        isUpload: ftpRequest.isUpload
                    };
                    let zeroEntry = this.getHistoryEntry(0, new Date(nowDate + 5), additionalData);
                    if (ftpRequest.isUpload) {
                        this.addCurrentUploadHistoryEntry(zeroEntry);
                    } else {
                        this.addCurrentDownloadHistoryEntry(zeroEntry);
